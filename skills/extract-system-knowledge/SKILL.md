...
先用一段话向用户复述要提取什么，避免漫无目的爬文档。两种入口形态都要支持：

- **单入口模式**：一个 URL → 它本身 + 它的子树
- **多入口模式**：一组兄弟 URL（围绕同一系统的多篇关联文档，无单一根页）→ 每个 URL 都当 `depth=0` 单独抓，可选地各自再递归子页

```
我将提取的"系统"是：<系统/模块名，例：Focus 工作台>
- 入口模式：<单入口 / 多入口（兄弟 URL N 个）>
...
1. 对每个入口 URL 调用合适的 MCP 工具，取入口页正文与元数据
   - 多入口模式下，所有入口都是 `depth=0` / `parent_pageId=null`，逐个抓
   - **目录型父页（正文为空）**：仍记入 `sources.json`（带 `note: "入口页，正文为空（目录型父页）"`），但**不参与概念/流程提取**，仅用作子页索引
2. 调 `child/page` REST API 列直接子页：
   - `size == 0`：当前是叶子页，**到此为止**，只用该入口内容即可
   - `size > 0`：对每个子页递归回到第 1 步（默认深度 2 层；**单次会话子文档总数 ≤ 30**，超出主动询问用户是否继续）
3. 每篇文档收集：`pageId` / `title` / `updated` / Markdown 要点 / 嵌入图片（图片用 vision 看一遍写要点）；遇到抓取异常（受权限、URL 形式特殊等）在 `sources.json` 对应页加 `note` 备注
...
- **KNOWLEDGE.md**：frontmatter **字段白名单**（仅这 7 个，多写的字段一律删除）：

  ```
  name / system / description / generated_at / generator / code_anchors[] / status
  ```

  其中 `description` 是一句话系统简介，≤ 80 字。**严禁**把页面清单（`sources` / `pages`）写进 frontmatter——那是 `sources.json` 的活，重复维护必出错。正文按 7 节顺序「系统定位 → 概念表 → 系统关系 → 核心流程 → 文档vs代码差异 → 未覆盖 → 出处索引」。完整模板见 [templates.md 第 4 节](templates.md)。

- **sources.json**：抓过的页面清单**唯一**存放处，只索引 `pageId / title / url / confluence_updated / depth / parent_pageId`（可选 `note`），**不存正文**。要重读时按 pageId 用 `get_confluence_content` 重抓。schema 见 [templates.md 第 5 节](templates.md)。
...
- **frontmatter 字段白名单**：KNOWLEDGE.md frontmatter 只允许 `name / system / description / generated_at / generator / code_anchors[] / status` 七个字段；页面清单**只**进 `sources.json`，绝不写进 frontmatter
...
- [ ] KNOWLEDGE.md frontmatter 字段齐全（`name` / `system` / `description` / `generated_at` / `status`），`description` ≤ 80 字
